...
def do_login():
    with sync_playwright() as p:
        ...
        print(f"\nSession saved to {AUTH_STATE_FILE}. You can now use 'read' commands.")
        browser.close()

# ---------------------------------------------------------------------------
# STEP 2: READ PAGE, INVENTORY SECTIONS & WRITE SELF-CONTAINED INTERACTIVE GRID
# ---------------------------------------------------------------------------
def read_page(url):
    ...
    with sync_playwright() as p:
        ...
        # Give dynamic diagrams, macro wrappers, and corporate iframes 3 seconds to fully render
        print("[⏳] Giving complex interactive charts and iframes time to render...")
    ...
    for i, heading in enumerate(heading_tags):
        ...
        # Aggressive Multi-Vector Diagram Detection
        has_direct_media = section_soup.find(['img', 'svg', 'iframe', 'canvas', 'object'])
        has_macro_wrapper = section_soup.select(
            ".confluence-embedded-image, .ak-renderer-extension, "
            "[data-macro-name='gliffy'], [data-macro-name='drawio'], "
            "[data-macro-name='viewdoc'], [data-macro-name='iframe'], "
            "div[class*='fabric-editor'], div[data-viewer='true'], "
            ".media-single-container, .media-card, [data-node-type='mediaSingle']"
        )
        
        if has_direct_media or has_macro_wrapper:
            img_count = len(section_soup.find_all(['img', 'iframe', 'canvas'])) or len(has_macro_wrapper) or 1
            detected_types.append(f"🖼️ Diagram/Image ({img_count}x)")
            
        # Source Code Code-Block Ingestion Tracking
        if section_soup.find('pre') or section_soup.select(".code, .syntaxhighlighter"):
            detected_types.append("💻 Code Block")
            
        # Text/Paragraph Character Block Check
    ...
    # -----------------------------------------------------------------------
    # COMPILING THE INNER TABLE ROW HTML SEGMENTS
    # -----------------------------------------------------------------------
    table_rows_html = ""
    ...
    # -----------------------------------------------------------------------
    # RAW STRUCTURE SHELL (Uses clean markers to prevent brackets conflict)
    # -----------------------------------------------------------------------
...
<head>
    ...
    <style>
        ...
        thead th { background-color: #f8fafc; position: sticky; top: 0; z-index: 10; padding: 14px 16px; color: #475569; font-weight: 700; border-bottom: 2px solid #e2e8f0; }tbody tr { border-bottom: 1px solid #f1f5f9; }tbody tr:hover { background-color: #f8fafc; }tbody tr.numbered-row { background-color: #ffffff; }tbody tr.appendix-row { background-color: #f8fafc; opacity: 0.85; }td { padding: 12px 16px; vertical-align: middle; }.id-cell { font-family: monospace; font-weight: 700; color: #0f6cbd; }.badge-level { background-color: #f1f5f9; color: #475569; padding: 2px 6px; border-radius: 4px; font-size: 11px; font-weight: 700; border: 1px solid #cbd5e1; }.title-cell { font-weight: 600; color: #1e293b; }.pill-container { display: flex; gap: 6px; flex-wrap: wrap; }.badge { padding: 4px 10px; border-radius: 20px; font-size: 12px; font-weight: 600; display: inline-flex; align-items: center; }.badge-table { background-color: #e0f2fe; color: #0369a1; border: 1px solid #bae6fd; }.badge-diagram { background-color: #f3e8ff; color: #6b21a8; border: 1px solid #e9d5ff; }.badge-code { background-color: #fef3c7; color: #92400e; border: 1px solid #fde68a; }.badge-text { background-color: #dcfce7; color: #166534; border: 1px solid #bbf7d0; }.badge-empty { background-color: #f1f5f9; color: #64748b; font-style: italic; font-size: 11px; }.section-checkbox { width: 16px; height: 16px; cursor: pointer; accent-color: #0f6cbd; }
...
"""
    ...
    # Apply substitutions directly to avoid python f-string conflict blocks
