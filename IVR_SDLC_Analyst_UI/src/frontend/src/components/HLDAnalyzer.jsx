...
export default function HLDAnalyzer() {
    const [wikiURL, setWikiURL] = useState(""); // 🟢 Replaces selectedFile state pointers
    ...
    // 1. Text URL Input Change Handler
    ...
    // 2. Manual Matrix Generation (Used for parsing pages)
    async function triggerPhase1MatrixIngestion() {
        ...
        try {
            const data = await generateHLDMatrix(wikiURL); //
    ...
    async function handleForcedCacheClear() {
        ...
        setLoading(true); //
        try {
            await clearHLDMatrixCache(wikiURL); //
            setVerificationMatrix(null); //
            setAnalysisResult(null); //
            setIsLoadedFromCache(false); //
    ...
    async function confirmAndTriggerLLMReview() {
        ...
        try {
            const data = await runCopilotHLDAudit(wikiURL); //
    ...
    return (
        <section className="hld-analyzer-panel" style={{ padding: "24px", fontFamily: "Segoe UI, sans-serif", display: "grid", gridTemplateColumns: "340px 1fr", gap: "24px" }}>
            ...
            <div style={{ background: "#fff", padding: "20px", borderRadius: "8px", border: "1px solid #e2e8f0", height: "fit-content", display: "flex", flexDirection: "column", gap: "16px" }}>
                <div>
                    <h3 style={{ margin: "0 0 4px 0", color: "#1e293b" }}>Confluence HLD Portal</h3>
                    <p style={{ margin: 0, fontSize: "0.8rem", color: "#64748b" }}>Live wiki parsing workbench workspace.</p>
                </div>

                {/* 🟢 CHANGED: Replaced file input fields with URL inputs */}
                <div style={{ background: "#f8fafc", padding: "12px", borderRadius: "6px", border: "1px dashed #cbd5e1" }}>
                    <label style={{ fontWeight: "600", fontSize: "0.85rem", display: "block", marginBottom: "8px" }}>Atlassian Wiki Page URL</label>
                    <input 
                        type="text" 
                        placeholder="https://reqcentral.com/wiki/spaces/..." 
                        value={wikiURL}
                        onChange={handleURLChange}
                        style={{ fontSize: "0.85rem", width: "100%", padding: "6px", border: "1px solid #cbd5e1", borderRadius: "4px" }} 
                    />
                </div>

                {!verificationMatrix ? (
                    <button onClick={triggerPhase1MatrixIngestion} disabled={loading || !wikiURL} style={{ background: "#0f6cbd", color: "#fff", padding: "10px", border: "none", borderRadius: "6px", fontWeight: "600", cursor: (loading || !wikiURL) ? "not-allowed" : "pointer", opacity: !wikiURL ? 0.6 : 1 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                ...
                {loading && (
                    <div style={{ padding: "40px", background: "#f8fafc", borderRadius: "8px", border: "1px dashed #cbd5e1", textAlign: "center", color: "#64748b", fontWeight: "500" }}>
                        Connecting to Atlassian Wiki Secure Node via Playwright execution layers... Please hold...
                    </div>
                )}
                ...
                {!verificationMatrix && !loading && (
                    <div style={{ background: "#f8fafc", border: "2px dashed #cbd5e1", borderRadius: "8px", padding: "60px", textAlign: "center", color: "#94a3b8" }}>
                        Enter a valid Confluence specification URL above to initialize your validation matrix canvas.
                    </div>
                )}
                    ...
                    <div style={{ background: "#fff", padding: "20px", borderRadius: "8px", border: "1px solid #e2e8f0" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", borderBottom: "1px solid #e2e8f0", paddingBottom: "12px" }}>
                            <div>
                                ...
                                <p style={{ margin: "2px 0 0 0", fontSize: "0.85rem", color: "#64748b" }}>Target Document: <span style={{ fontFamily: "monospace", fontWeight: "600", wordBreak: "break-all" }}>{wikiURL}</span></p>
                            </div>
                                        ...
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
                                            ...
                                            <div style={{ display: "flex", gap: "4px" }}>
                                                {chunk.metadata.audit_tags.map((tag, tIdx) => {
                                                    const isSecurity = tag.toUpperCase() === "SECURITY";
                                                    const isIntegration = tag.toUpperCase() === "INTEGRATION";
                                                    ...
                                                    return (
                                                        <span 
                                                            ...
                                                            style={{ 
                                                                fontSize: "0.72rem", 
                                                                background: isSecurity ? "#fef2f2" : isIntegration ? "#e0f2fe" : "#f1f5f9", 
                                                                color: isSecurity ? "#ef4444" : isIntegration ? "#0369a1" : "#475569", 
                                                                padding: "2px 8px", 
                                                                borderRadius: "4px", 
                                                                fontWeight: "700", 
                                                                border: `1px solid ${isSecurity ? "#fca5a5" : isIntegration ? "#bae6fd" : "#cbd5e1"}`
                                                            }}
                                                        >
                                                            {tag}
                                                        </span>
                                    ...
                                    analysisResult.ivr_flow_requirements.map((flow, idx) => (
                                        <div key={idx} style={{ background: "#fff", border: "1px solid #cbd5e1", padding: "12px", borderRadius: "6px", marginBottom: "8px" }}>
                                            ...
                                            <ul style={{ margin: "6px 0 0 0", paddingLeft: "16px", fontSize: "0.85rem", color: "#334155" }}>
                                                {flow.intended_steps && flow.intended_steps.length > 0 ? (
                                                    <ul style={{ margin: "6px 0 0 0", paddingLeft: "16px", fontSize: "0.85rem", color: "#334155" }}>
                                                        {flow.intended_steps.map((step, sIdx) => (
                                                            <li key={sIdx} style={{ marginBottom: "2px" }}>{step}</li>
                                                        ))}
                                                    </ul>
                                                ) : (
                                                    <div style={{ fontSize: "0.8rem", fontStyle: "italic", color: "#94a3b8", marginTop: "4px", paddingLeft: "4px" }}>
                                                        No operational instruction sub-steps defined for this route.
                                                    </div>
                                                )}
                                            </ul>
                                        </div>
